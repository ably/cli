import { AblyCliTerminal, AblyCliTerminalHandle } from "@ably/react-web-cli";
...
const getInitialCredentials = () => {
  ...
  const rememberCredentials =
    localStorage.getItem(`ably.web-cli.rememberCredentials.${wsDomain}`) ===
    "true";
  if (rememberCredentials) {
    ...
    const storedAccessToken = localStorage.getItem(
      `ably.web-cli.accessToken.${wsDomain}`,
    );
    if (storedApiKey) {
      return {
        apiKey: storedApiKey,
  ...
  const sessionApiKey = sessionStorage.getItem(
    `ably.web-cli.apiKey.${wsDomain}`,
  );
  ...
  if (qsApiKey) {
    if (isProduction) {
      console.error(
        "Security Warning: API keys in query parameters are not allowed in production environments.",
      );
...
function App() {
  ...
  const [connectionStatus, setConnectionStatus] =
    useState<TermStatus>("disconnected");
  const [displayMode, setDisplayMode] = useState<"fullscreen" | "drawer">(
    initialMode,
  );
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(
    Boolean(initialCreds.apiKey && initialCreds.apiKey.trim()),
  );
  const [isUsingCustomAuth, setIsUsingCustomAuth] = useState(
    initialCreds.source === "session" || initialCreds.source === "localStorage",
  );
  ...
  const handleAuthenticate = useCallback(
    (newApiKey: string, newAccessToken: string, remember?: boolean) => {
      ...
      const shouldRemember =
        remember !== undefined ? remember : rememberCredentials;
  ...
  const handleAuthSettingsSave = useCallback(
    (newApiKey: string, newAccessToken: string, remember: boolean) => {
      if (newApiKey) {
      ...
      isAuthenticated && apiKey && apiKey.trim() ? (
        <AblyCliTerminal
          ...
          maxReconnectAttempts={
            5
          } /* In the example, limit reconnection attempts for testing, default is 15 */
    ...
    <div className="App fixed">
      ...
      <header className="App-header">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button onClick={() => terminalRef.current?.toggleSplit()}>
            Toggle split
          </button>
        </div>
      </header>
