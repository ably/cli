...
export const AblyCliTerminal = React.forwardRef<
  AblyCliTerminalHandle,
  AblyCliTerminalProps
>(
  (
    {
      websocketUrl,
    ...
    const handleCloseSplit = useCallback(() => {
      ...
      if (
        secondarySocketRef.current &&
        secondarySocketRef.current.readyState < WebSocket.CLOSING
      ) {
    ...
    const handleClosePrimary = useCallback(() => {
      ...
      if (secondaryTerm.current && secondarySocketRef.current) {
        debugLog(
          "[AblyCLITerminal] Closing primary terminal, keeping secondary",
        );
    ...
    const [secondarySessionId, setSecondarySessionId] = useState<string | null>(
      () => {
        if (
          resumeOnReload &&
          typeof window !== "undefined" &&
          window.sessionStorage.getItem("ably.cli.isSplit") === "true"
        ) {
    ...
    const updateConnectionStatusAndExpose = useCallback(
      (status: ConnectionStatus) => {
        ...
        if (
          typeof (window as any).setWindowTestFlagOnStatusChange === "function"
        ) {
    ...
    const handlePtyData = useCallback(
      (data: string) => {
        ...
        const sanitizedData = data
          ...
          .replace(/\t/g, "\\t");
        debugLog(
          `⚠️ DIAGNOSTIC: handlePtyData called. isSessionActive: ${isSessionActiveRef.current}, data: "${sanitizedData}"`,
        );
        ...
        if (!isSessionActiveRef.current) {
          ...
          debugLog(
            `⚠️ DIAGNOSTIC: Current connection status: ${connectionStatusRef.current}, sessionId: ${sessionIdRef.current}`,
          );
          ...
          const cleanBuf = ptyBuffer.current.replace(
            /\u001B\[[0-9;]*[mGKHF]/g,
            "",
          );
          debugLog(
    ...
    const secondarySpinnerIntervalRef = useRef<ReturnType<
      typeof setInterval
    > | null>(null);
    ...
    const handleControlMessage = useCallback(
      (msg: any) => {
        if (msg.type === "hello" && typeof msg.sessionId === "string") {
          debugLog(
            `⚠️ DIAGNOSTIC: Received hello message with sessionId=${msg.sessionId}`,
          );
        ...
        if (msg.type === "status") {
          debugLog(
            `⚠️ DIAGNOSTIC: Received server status message: ${msg.payload}`,
          );
    ...
    const handleSecondaryPtyData = useCallback(
      (data: string) => {
        if (!isSecondarySessionActive) {
          ...
          // Strip ANSI colour/formatting codes before looking for the prompt
          const cleanBuf = secondaryPtyBuffer.current.replace(
            /\u001B\[[0-9;]*[mGKHF]/g,
            "",
          );
      ...
      (isRetry: boolean) => {
        ...
        if (term.current) {
          ...
          const initialSpinnerChar =
            spinnerFrames[spinnerIndexRef.current % spinnerFrames.length];
        ...
        const initialSpinnerChar =
          spinnerFrames[spinnerIndexRef.current % spinnerFrames.length];
        const initialLines = [
          `${initialSpinnerChar} ${spinnerPrefixRef.current}`,
          "",
    ...
    const connectWebSocket = useCallback(() => {
      ...
      if (
        !showManualReconnectPromptRef.current &&
        socketRef.current &&
        socketRef.current.readyState === WebSocket.OPEN
      ) {
        if ((window as any).ABLY_CLI_DEBUG)
          console.warn(
      ...
      debugLog(
        `⚠️ DIAGNOSTIC: New WebSocket created with ID: ${Math.random().toString(36).substring(2, 10)}`,
      );
    ...
    const handleWebSocketMessage = useCallback(
      async (event: MessageEvent) => {
        ...
        try {
          ...
          debugLog(
            `⚠️ DIAGNOSTIC: Processing as terminal output: "${dataStr}"`,
          );
          ...
          if (filteredData.length > 0) {
            ...
            if (filteredData.includes("ABLY_CTRL:")) {
              ...
              while (currentPos < filteredData.length) {
                const ctrlIndex = filteredData.indexOf(
                  "ABLY_CTRL:",
                  currentPos,
                );
                ...
                if (ctrlIndex > currentPos) {
                  regularOutput += filteredData.substring(
                    currentPos,
                    ctrlIndex,
                  );
                }
                ...
                if (msgEnd === -1)
                  msgEnd = filteredData.indexOf("\r", ctrlIndex);
    ...
    const handleWebSocketError = useCallback(
      (event: Event) => {
        console.error(
          "[AblyCLITerminal] WebSocket error event received:",
          event,
        );
    ...
    useEffect(() => {
      ...
      if (!term.current && rootRef.current) {
        ...
        debugLog(
          "⚠️ DIAGNOSTIC: Setting up custom key handler for special keys",
        );
        ...
        term.current.onData((data: string) => {
          ...
          if (data === "\r") {
            ...
            debugLog(
              `⚠️ DIAGNOSTIC: Enter key pressed, status: ${latestStatus}, reconnectPrompt: ${showManualReconnectPromptRef.current}`,
            );
        ...
        grSetCountdownCallback((remainingMs) => {
          ...
          if (
            ...
            statusBoxRef.current.content.length > 1
          ) {
      ...
      if (overlay && rootRef.current) {
        ...
        requestAnimationFrame(() => {
          try {
            ...
            const overlayEl = rootRef.current?.querySelector(
              ".ably-overlay",
            ) as HTMLElement | null;
    ...
    useEffect(() => {
      const initializeSession = async () => {
        ...
        const storedHash = window.sessionStorage.getItem(
          `ably.cli.credentialHash.${urlDomain}`,
        );
      ...
      newSocket.addEventListener("message", async (event) => {
        try {
          ...
          if (msg) {
            ...
            if (msg.type === "hello" && typeof msg.sessionId === "string") {
              debugLog(
                `[Secondary] Received hello. sessionId=${msg.sessionId}`,
              );
            ...
            if (msg.type === "status") {
              debugLog(
                `[Secondary] Received server status message: ${msg.payload}`,
              );
              ...
              if (msg.payload === "error" || msg.payload === "disconnected") {
                ...
                updateSecondaryConnectionStatus(
                  msg.payload as ConnectionStatus,
                );
                ...
                if (secondaryTerm.current && msg.payload === "disconnected") {
                  ...
                  if (secondaryTerm.current) {
                    secondaryStatusBoxRef.current = drawBox(
      ...
      secondaryTerm.current.attachCustomKeyEventHandler(
        (event: KeyboardEvent): boolean => {
          ...
          if (
            ...
            !secondarySocketRef.current ||
            secondarySocketRef.current.readyState !== WebSocket.OPEN
          ) {
            return true; // Let xterm handle it normally
          }
          ...
          if (event.key === "Tab" && !event.ctrlKey && !event.altKey) {
            ...
            secondarySocketRef.current.send(
              JSON.stringify({
                type: "readline-control",
                action: "complete",
      ...
      secondaryTerm.current.onData((data: string) => {
        ...
        if (data === "\r") {
          ...
          if (secondaryShowManualReconnectPromptRef.current) {
            ...
            if (
              secondarySocketRef.current &&
              secondarySocketRef.current.readyState !== WebSocket.CLOSED
            ) {
              try {
                secondarySocketRef.current.close(1000, "manual-reconnect");
              } catch {
                /* ignore */
              }
