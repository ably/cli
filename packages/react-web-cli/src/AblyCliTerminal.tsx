...
export const AblyCliTerminal: React.FC<AblyCliTerminalProps> = ({
  ...
  useEffect(() => {
    ...
    const debouncedWindowResizeFit = debounce(() => {
      if (isMounted && fitAddon.current) {
        // Step 4: Re-enable fitAddon check
    ...
    if (terminalRef.current && !term.current) {
      ...
      terminal.onResize(({ cols, rows }) => {
        if (isMounted && sessionStateRef.current === "active") {
  ...
  useEffect(() => {
    if (sessionState === "active" && term.current) {
      console.log(
        "[AblyCLITerminal] Session active - focusing terminal and triggering initial resize send",
      );
