...
import {
  AblyCliTerminal,
  type AblyCliTerminalHandle,
} from "@ably/react-web-cli";
  ...
  return (
    <div style={{ height: 500 }}>
      <div style={{ marginBottom: 8 }}>
        ...
        <button
          onClick={() => termRef.current?.toggleSplit()}
          style={{ marginLeft: 8 }}
        >
          Toggle split
        </button>
        <button
          onClick={() => termRef.current?.closeSplit()}
          style={{ marginLeft: 8 }}
        >
          Close split
        </button>
...
Enable it in any of the following ways _before_ the component mounts:
