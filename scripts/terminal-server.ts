...
function logError(message: unknown): void {
  console.error(
    `[TerminalServerError] ${message instanceof Error ? message.message : String(message)}`,
  );
...
async function _handleAuth(
  ...
  try {
    ...
    logError(
      `Error during authentication for session ${sessionId}: ${error instanceof Error ? error.message : String(error)}`,
    );
    if (
      ws.readyState === WebSocket.OPEN ||
      ws.readyState === WebSocket.CONNECTING
...
async function startServer() {
  ...
  const maxSessions = Number.parseInt(
    process.env.MAX_SESSIONS || String(DEFAULT_MAX_SESSIONS),
    10,
  );
    ...
    ws.once("message", async (message: Buffer) => {
      ...
      try {
        ...
        if (
          ...
          !isValidToken(authPayload.accessToken)
        ) {
        ...
        try {
          ...
          logError(
            `[${sessionId}] Failed to create container: ${containerError instanceof Error ? containerError.message : String(containerError)}`,
          );
          if (ws.readyState === WebSocket.OPEN) {
        ...
        logError(
          `[${sessionId}] Error during connection setup: ${error instanceof Error ? error.message : String(error)}`,
        );
        if (ws.readyState === WebSocket.OPEN) {
...
function pipeStreams(wsStream: Duplex, containerStream: stream.Duplex) {
  ...
  wsStream.pipe(containerStream).on("error", (err) => {
    logError(
      `Error piping WebSocket stream to container stream: ${err.message}`,
    );
  });
...
async function attachToContainer(
  ...
  try {
    ...
    log(
      `Attaching to container ${session.container.id} for session ${session.sessionId}`,
    );
    ...
    log(
      `Attached stream to container ${session.container.id} for session ${session.sessionId}`,
    );
    ...
    wsStream.on("error", (err) => {
      logError(
        `WebSocket stream error for session ${session.sessionId}: ${err.message}`,
      );
      if (containerStream) safeCloseWsStream(containerStream);
      cleanupSession(session.sessionId);
    });
    ...
    containerStream.on("error", (err) => {
      logError(
        `Container stream error for session ${session.sessionId}: ${err.message}`,
      );
    ...
    logError(
      `Failed to attach to container for session ${session.sessionId}: ${error instanceof Error ? error.message : String(error)}`,
    );
    safeCloseWsStream(wsStream);
    if (containerStream) {
  ...
  try {
    ...
    if (typeof parsed === "object" && parsed !== null) {
      if (
        ...
        if (
          typeof resizeData.cols === "number" &&
          typeof resizeData.rows === "number"
        ) {
          handleExecResize(session, {
            cols: resizeData.cols,
            rows: resizeData.rows,
          });
        } else {
          logError(
            `Received resize message with invalid data format for session ${session.sessionId}`,
          );
        }
      } else if (
        "type" in parsed &&
        ...
        if (session.stdinStream && !session.stdinStream.destroyed) {
          ...
          logError(
            `Cannot write data: stdinStream not available or destroyed for session ${session.sessionId}`,
          );
        }
      } else {
        logError(
          `Received unknown message type or format for session ${session.sessionId}: ${msgStr}`,
        );
      }
    } else {
      logError(
        `Received non-object message after JSON parse for session ${session.sessionId}: ${msgStr}`,
      );
    }
  } catch {
    ...
    if (session.stdinStream && !session.stdinStream.destroyed) {
      ...
      logError(
        `Cannot write raw data: stdinStream not available or destroyed for session ${session.sessionId}`,
      );
    }
  }
}
