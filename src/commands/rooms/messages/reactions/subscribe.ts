      ...
      if (flags.raw) {
        ...
        this.unsubscribeRawReactionsFn = room.messages.reactions.subscribeRaw((event: MessageReactionRawEvent) => {
          ...
          if (this.shouldOutputJson(flags)) {
            ...
            // The SDK might not include metadata in the reaction
            const metadata = (event.reaction as any).metadata;
            if (metadata && Object.keys(metadata).length > 0) {
              this.log(
                `  ${chalk.dim("Metadata:")} ${this.formatJsonOutput(metadata, flags)}`,
              );
            }
          }
        });
